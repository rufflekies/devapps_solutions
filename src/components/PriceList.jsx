...
const Pricelist = () => {
  ...
  return (
    <div className="bg-gray-100 pt-10 font-montserrat">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-4xl font-bold text-center">Paket Harga</h2>

        {/* Horizontal scrolling for mobile view only */}
        <div className="overflow-x-auto md:hidden pt-8">
          <div className="flex space-x-4 pb-4">
            {pricingData.map((plan, index) => (
              <div key={index} className="bg-white border rounded-3xl p-6 shadow-md transition-transform transform hover:scale-105 flex-shrink-0 w-80 flex flex-col">
                <div className="flex-grow">
                  <h3 className="text-xl font-semibold text-center">{plan.plan}</h3>
                  <div className="text-center">
                    <span className="text-3xl font-bold">
                      <CountUp start={0} end={plan.price} duration={1} separator="." prefix="Rp " />
                    </span>
                  </div>
                  <p className="text-lg font-normal mb-6 text-center">/proyek</p>
        ...
        <div className="hidden md:grid md:gap-8 md:pt-8 md:grid-cols-2 lg:grid-cols-3">
          {pricingData.map((plan, index) => (
            <div key={index} className="flex flex-col bg-white border rounded-3xl p-6 shadow-md transition-transform transform hover:scale-105">
              <div className="flex-grow">
